...
const Departments = () => {
  ...
  const handleRowAdd = (newData, resolve) => {
    //validation
    let errorList = [];
    if (newData.station === undefined) {
      errorList.push("Please enter station, department or unit  ");
      setIserror(true);
    }
    ...
    if (errorList.length < 1) {
      //no error
      axios
        .post("http://tmsapi.db/api/department/store", newData)
        .then((response) => {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    if (errorList.length < 1) {
      axios
        .post(`http://tmsapi.db/api/department/update/${oldData.id}`, newData)
        .then((response) => {
